...
const{authorise}=require("../middlewares/authories")
...
userRouter.post("/login",async(req,res,next)=>{
  ...
  userRouter.get("/getnewtoken",(req,res)=>{
    ...
    userRouter.get("/notes", authMiddleware,redisclient,(req, res) => {
        res.send("notes.......")
      })
      ...
      userRouter.get("/logout", async (req, res) => {
        //we are splitting the headers because we want to have multiple tokens there
        // blacklist.push(req.headers?.authorization?.split(" ")[1]);
      //   const payload= req.headers.authorization.split(" ")[1];
      //   const blacklist= new blackListModel({token:payload});
      ...
      // userRouter.get("/home-page",(req,res)=>{
      //   let x=req.cookies.email
      //   console.log(x)
      //   res.send("successfully all work")
